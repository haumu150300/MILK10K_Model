...
def train_batch(model: torch.nn.Module, batch, criterion, optimizer, scheduler, device: torch.device):
    ...
    with torch.amp.autocast(device_type=device.type):
        outputs = model(input1, metadata1)
...
if __name__ == "__main__":
    ...
    config = Config()
    config.model_saved_path = "./swin_chkpt"
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=3,  # try 2 or 4 in Colab
        pin_memory=True,
        persistent_workers=True,
    )
    torch.backends.cudnn.benchmark = True
    model = Swin(image_size=256, num_classes=11)
    ...
    #     model, optimizer, config, device, saved_checkpoint=None
    # )
    ...
    #     pin_memory=True,
    # )
    writer = SummaryWriter(log_dir="./runs/swin")
    ...
    with tqdm.tqdm(
        ...
        for epoch in range(init_step, total_steps):
            try:
                ...
                data_iter = iter(train_loader)
                batch = next(data_iter)
            train_loss = train_batch(model, batch, criterion, optimizer, None, device)
            # val_loss, val_accuracy = val_data(model, val_loader, criterion, device)
            val_loss, val_accuracy = 0.0, 0.0
            pbar.update(1)
            print(
                f"Epoch {epoch+1}/{total_steps}, Train Loss: {train_loss:.5f}"
            )
